...
const TYPE_BADGE = {
  mcq: { bg: "rgba(34,211,238,0.10)", border: "rgba(34,211,238,0.25)", text: "#22d3ee" },
  cq: { bg: "rgba(168,85,247,0.10)", border: "rgba(168,85,247,0.25)", text: "#a855f7" },
};

function typeBadge(type) {
  const t = type?.toLowerCase();
  const cfg = TYPE_BADGE[t] || { bg: "rgba(100,116,139,0.12)", border: "#334155", text: "#94a3b8" };
  return (
    <span style={{
      fontSize: 10,
      fontWeight: 700,
...
export default function ArchiveQuestionPicker({ open, onClose, selectedIds = [], onApply }) {
  ...
  useEffect(() => { setLocalSelected(selectedIds); }, [selectedIds]);
  ...
  // Filter questions by type and search
    ...
    return archives.map((archive) => {
      ...
      archiveQuestions.forEach((q) => {
        ...
        if (!chapterMap.has(chapterId)) {
          chapterMap.set(chapterId, { chapterId, chapterName, questions: [] });
        }
      ...
      <div className="stack">
          ...
          <div style={{ display: "flex", gap: 4, background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: 3 }}>
            {TYPE_TABS.map((tab) => {
              const count =
                tab.key === "all"
                  ? questions.length
                  : questions.filter((q) => q.type?.toLowerCase() === tab.key).length;
              return (
                <button
                  key={tab.key}
                  type="button"
                  ...
                  style={{
                    ...
                    border: typeTab === tab.key ? "1px solid #2563eb" : "1px solid transparent",
                    borderRadius: 6,
                ...
                >
                  {tab.label}
                  <span style={{
                    background: typeTab === tab.key ? "rgba(147,197,253,0.15)" : "rgba(255,255,255,0.04)",
                    borderRadius: 4,
                    padding: "0 5px",
                    fontSize: 10,
                    fontWeight: 700,
                    color: typeTab === tab.key ? "#93c5fd" : "#475569",
                  }}>
                    {count}
                  </span>
                </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 440, overflowY: "auto", paddingRight: 2 }}>
          {groupedArchives.length === 0 ? (
            <div style={{ textAlign: "center", padding: "32px 0", color: "#475569" }}>
              <p style={{ margin: 0, fontSize: 13 }}>No questions match this filter.</p>
            </div>
          ) : (
            groupedArchives.map((archive) => {
              ...
              return (
                <div
                  key={archive._id}
                  style={{ border: "1px solid #1e293b", borderRadius: 10, overflow: "hidden", background: "#0d1929" }}
                >
                  {/* Archive header */}
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span style={{ fontSize: 11, color: "#475569", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 5, padding: "2px 8px" }}>
                      {archive.total} Q
                    </span>
                  </button>
                  ...
                  {/* Chapters */}
                  {isOpen && (
                    <div style={{ borderTop: "1px solid #1e293b", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
                      {archive.chapters.map((chapter) => {
                        const isChOpen = !!openChapters[chapter.chapterId];
                        const selectedInChapter = chapter.questions.filter((q) => localSelected.includes(q._id)).length;
                        const allChapterSelected = chapter.questions.length > 0 && selectedInChapter === chapter.questions.length;

                        return (
                          <div
                            key={chapter.chapterId}
                            style={{ border: "1px solid #1e293b", borderRadius: 8, overflow: "hidden", background: "#0f172a" }}
                          >
                            {/* Chapter header */}
                            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px" }}>
                              ...
                              >
                                <span style={{ color: "#334155", lineHeight: 0 }}>
                                  {isChOpen ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                                </span>
                                <span style={{ fontSize: 12, fontWeight: 600, color: "#cbd5e1" }}>
                                  {chapter.chapterName}
                                </span>
                                {selectedInChapter > 0 && (
                                  <span style={{ fontSize: 10, color: "#4ade80", background: "rgba(74,222,128,0.08)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: 4, padding: "0 6px" }}>
                              ...
                              <button
                                type="button"
                                onClick={() => toggleAllInChapter(chapter.questions)}
                                style={{
                                  background: "none",
                                  border: "1px solid #1e293b",
                                  borderRadius: 5,
                                  color: allChapterSelected ? "#4ade80" : "#475569",
                                  fontSize: 10,
                                  fontWeight: 700,
                                  padding: "2px 8px",
                                  cursor: "pointer",
                              ...
                              >
                                {allChapterSelected ? "Deselect All" : "Select All"}
                              </button>
                            </div>

                            {/* Questions */}
                            {isChOpen && (
                              <div style={{ borderTop: "1px solid #1e293b", padding: "8px 10px", display: "flex", flexDirection: "column", gap: 4 }}>
                                  ...
                                  return (
                                    <label
                                      ...
                                      style={{
                                        ...
                                        border: `1px solid ${checked ? "rgba(37,99,235,0.3)" : "transparent"}`,
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid #1e293b", paddingTop: 14 }}>
          <span style={{ fontSize: 12, color: "#64748b" }}>
            {selectedCount > 0 ? (
              <span style={{ color: "#4ade80", fontWeight: 700 }}>{selectedCount} question{selectedCount !== 1 ? "s" : ""} selected</span>
            ) : (
